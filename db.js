...
export async function loginWithEmail(email, password) {
    try {
        ...
        if (email === ADMIN_EMAIL) {
            const adminData = { uid: user.uid, email: user.email, name: 'المشرف', isAdmin: true };
...
export async function loginAsAdmin(password) {
    if (password === 'admin123') {
        const adminData = { uid: 'admin', email: ADMIN_EMAIL, name: 'المشرف', isAdmin: true };
        localStorage.setItem('currentUser', JSON.stringify(adminData));
        return { success: true, user: adminData };
    }
...
export async function loginWithPhone(phone) {
    try {
        ...
        if (snapshot.empty) return { success: false, message: 'رقم الجوال غير مسجل أو بانتظار الموافقة' };
...
export async function registerWithPhone(phone, name) {
    try {
        ...
        const snapshot = await getDocs(q);
        if (!snapshot.empty) return { success: false, message: 'هذا الرقم مسجل مسبقاً' };
        await addDoc(collection(db, 'users'), { phone, name, approved: false, createdAt: new Date().toISOString() });
